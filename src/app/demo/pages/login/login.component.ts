  ...
  onForgotPassword(event: Event) {
    ...
    Swal.fire({
      ...
      preConfirm: (username) => {
        ...
        return this.recuperarPasswordService.recuperarPassword(username).toPromise()
          .then((response) => {
            return true;
          })
          .catch((error) => {
            Swal.showValidationMessage('No se pudo enviar el correo de recuperación.');
            return false;
          });
      },
      ...
      if (result.isConfirmed) {
        Swal.fire({
          ...
          text: 'Si el usuario existe, se enviará un correo con instrucciones para recuperar la contraseña.',
          icon: 'success'
        });
      }
    });
